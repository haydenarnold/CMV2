...
def get_mft_values_to_run(
    ...
    start_index = FOUNDATIONS.index(start_mft)

    return FOUNDATIONS[start_index:]
...
def main() -> None:
    ...
    output_path = (
        output_dir
        / f"dataset_{args.year}_scored_sft.feather"
    )

    checkpoint_path = (
        output_dir
        / f"dataset_{args.year}_scored_checkpoint_sft.feather"
    )
    ...
    if device.type == "cuda":
        ...
        print("GPU: MPS device selected")
    else:
        print(
            "No GPU available. Batched inference will run on CPU."
        )
    ...
    for mft in mft_values_to_run:
        ...
        model = None
        try:
            model_dir = models_dir / mft.replace(" ", "_")
            ...
            # Report and continue to next foundation without crashing the whole run.
            print(f"Error while processing {mft}: {e}", flush=True)

        finally:
            # Safe cleanup: delete model only if it was created.
